...
public class frmDentista extends javax.swing.JInternalFrame {
    ...
    private int dentistaAtual = 0;
    private boolean alterarDentista = false;
    ...
    private void cmdsalvarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdsalvarActionPerformed
        ...
        //Salvando dentista no banco de dados

       
       Dentista d = new Dentista(Integer.parseInt(txtid.getText()),
                                 txtnome.getText(),
                                 txtendereco.getText(),
                                 txtcidade.getText(),
                                 txtbairro.getText(),
       ...
       boolean dentistaExiste = den.pesquisar(Integer.parseInt(txtid.getText()), bd);
       
        if(dentistaExiste && this.alterarDentista){
           this.alterarDentista = false;
           den.editar(dentistaAtual, d, bd);
           JOptionPane.showMessageDialog(rootPane, "Dentista editado com sucesso");
           return;
       }
       
       if(dentistaExiste){
           JOptionPane.showMessageDialog(rootPane, "Dentista já cadastratdo no sistema");
           return;
       } else {
          den.cadastrar(d, bd);
          JOptionPane.showMessageDialog(rootPane, "Dentista cadastratdo com sucesso");
          //return;
       }
    ...
    private void cmdprimeiroActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdprimeiroActionPerformed
        ...
        cmdcancelar.setEnabled(true);
        cmdbuscar.setEnabled(true);
           if(bd.getDentistas().size() == 0){
            JOptionPane.showMessageDialog(rootPane, "Não existe dados no banco! ");
            return;
        }
        vizualizarDentistas(0);
        this.dentistaAtual = 0;
    ...
    private void cmdultimoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdultimoActionPerformed
        ...
        vizualizarDentistas(bd.retornaIndiceUltimoCadastro());
        this.dentistaAtual = bd.retornaIndiceUltimoCadastro();
    ...
    private void cmdanteriorActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdanteriorActionPerformed
        ...
        this.dentistaAtual = this.dentistaAtual - 1;
        ...
        if(this.dentistaAtual < 0){
            this.dentistaAtual = 0;
            vizualizarDentistas(this.dentistaAtual);
            return;
        }
    ...
    private void cmdproximoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdproximoActionPerformed
        ...
        //Usado para mostrar os dados do dentista para o usuário
        //se ao clicar no botão proximo e o dentista atual exceder o valor do array criado
        //para não ocorrer erro, deve entrar nesse if
        if(this.dentistaAtual > bd.retornaIndiceUltimoCadastro()){
           vizualizarDentistas(bd.retornaIndiceUltimoCadastro()); 
           this.dentistaAtual = bd.retornaIndiceUltimoCadastro();
           return;
        }
    ...
    private void cmddeletarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmddeletarActionPerformed
        den.excluir(dentistaAtual, bd);
        JOptionPane.showMessageDialog(rootPane, "Dentista excluido com sucesso");
    }//GEN-LAST:event_cmddeletarActionPerformed

    private void cmdbuscarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdbuscarActionPerformed
        
        String coddentista = JOptionPane.showInputDialog("Favor digitar o código do dentista: ");
        
        if(coddentista.equals("")){
            return;
        }
        
        boolean flag = bd.verificaSeDentistaExiste(Integer.valueOf(coddentista));
        
        if(!flag){
            JOptionPane.showMessageDialog(rootPane, "Dentista não encontrado");
            return;
        }

        dentistaAtual = bd.retornaPosDentista(Integer.valueOf(coddentista));
        vizualizarDentistas(dentistaAtual);
    }//GEN-LAST:event_cmdbuscarActionPerformed

    //mostra os dados do dentista no formulario
    private void vizualizarDentistas(int indice){
        
        txtid.setText(Integer.toString(bd.getDentistas().get(indice).getId()));
        txtnome.setText(bd.getDentistas().get(indice).getNome());
        txtendereco.setText(bd.getDentistas().get(indice).getEndereco());
        txtcidade.setText(bd.getDentistas().get(indice).getCidade());
        txtbairro.setText(bd.getDentistas().get(indice).getBairro());
        txtrg.setText(bd.getDentistas().get(indice).getRg());
        txtcpf.setText(bd.getDentistas().get(indice).getCpf());
        txtemail.setText(bd.getDentistas().get(indice).getEmail());
        txttelfixo.setText(Integer.toString(bd.getDentistas().get(indice).getTelFixo()));
        txttelcel.setText(Integer.toString(bd.getDentistas().get(indice).getTelCel()));
        txtsalario.setText(Double.toString(bd.getDentistas().get(indice).getSalario()));
        jdadatapagamento.setDate(bd.getDentistas().get(indice).getDataPagamento());
        txtsenha.setText(bd.getDentistas().get(indice).getSenha());
