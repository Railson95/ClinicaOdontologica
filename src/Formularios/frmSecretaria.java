 ...
 * @author rails
 */
public class frmSecretaria extends javax.swing.JInternalFrame {

    private Dados bd;
    private Dentista den;
    private Secretaria ass;
    private int SecretariaAtual = 0;
    private boolean alterarSecretaria = false;
    private Paciente pAuxuliar;
        
    public void atualizarDados(Dados bd, Dentista den, Paciente pAuxiliar, Secretaria aAuxiliar){
        this.bd = bd;
        this.den = den;
    ...
    private void cmdsalvarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdsalvarActionPerformed
        ...
        //Salvando Secretaria no banco de dados

       
       Secretaria a = new Secretaria(Integer.parseInt(txtid.getText()),
                                 txtnome.getText(),
                                 txtendereco.getText(),
                                 txtcidade.getText(),
                                 txtbairro.getText(),
       ...
       boolean SecretariaExiste = den.pesquisar(Integer.parseInt(txtid.getText()), bd);
       
        if(SecretariaExiste && this.alterarSecretaria){
           this.alterarSecretaria = false;
           a.editar(SecretariaAtual, a, bd);
           JOptionPane.showMessageDialog(rootPane, "Secretaria editada com sucesso");
           return;
       }
       
       if(SecretariaExiste){
           JOptionPane.showMessageDialog(rootPane, "Secretaria já cadastratda no sistema");
           return;
       } else {
          a.cadastrar(a, bd);
          JOptionPane.showMessageDialog(rootPane, "Secretaria cadastratda com sucesso");
          //return;
       }
    ...
    private void cmdprimeiroActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdprimeiroActionPerformed
        ...
        if(bd.getSecretarias().size() == 0){
            JOptionPane.showMessageDialog(rootPane, "Não existe dados no banco! ");
            return;
        }
        vizualizarSecretaria(0);
        this.SecretariaAtual = 0;
    ...
    private void cmdultimoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdultimoActionPerformed
        ...
        vizualizarSecretaria(bd.retornaIndiceUltimoCadastroSecretaria());
        this.SecretariaAtual = bd.retornaIndiceUltimoCadastroSecretaria();
    ...
    private void cmdanteriorActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdanteriorActionPerformed
        ...
        this.SecretariaAtual = this.SecretariaAtual - 1;
        
        //Usado para mostrar os dados do Secretaria para o usuário
        //se ao clicar no botão anterior e o Secretaria atual valer -1 que é uma posição fora do Array
        //para não ocorrer erro, deve entrar nesse if
        if(this.SecretariaAtual < 0){
            this.SecretariaAtual = 0;
            vizualizarSecretaria(this.SecretariaAtual);
            return;
        }
    ...
    private void cmdproximoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdproximoActionPerformed
        ...
        //Usado para mostrar os dados do Secretaria para o usuário
        //se ao clicar no botão proximo e o Secretaria atual exceder o valor do array criado
        //para não ocorrer erro, deve entrar nesse if
        if(this.SecretariaAtual > bd.retornaIndiceUltimoCadastroSecretaria()){
           vizualizarSecretaria(bd.retornaIndiceUltimoCadastroSecretaria()); 
           this.SecretariaAtual = bd.retornaIndiceUltimoCadastroSecretaria();
           return;
        }
    ...
    private void cmddeletarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmddeletarActionPerformed
        ass.excluir(SecretariaAtual, bd);
        JOptionPane.showMessageDialog(rootPane, "Secretaria excluido com sucesso");
    }//GEN-LAST:event_cmddeletarActionPerformed

    private void cmdbuscarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdbuscarActionPerformed
        
        String codSecretaria = JOptionPane.showInputDialog("Favor digitar o código do Secretaria: ");
        
        if(codSecretaria.equals("")){
            return;
        }
        
        boolean flag = bd.verificaSeSecretariaExiste(Integer.valueOf(codSecretaria));
        
        if(!flag){
            JOptionPane.showMessageDialog(rootPane, "Secretaria não encontrado");
            return;
        }

        SecretariaAtual = bd.retornaPosSecretaria(Integer.valueOf(codSecretaria));
        vizualizarSecretaria(SecretariaAtual);
    }//GEN-LAST:event_cmdbuscarActionPerformed

    //mostra os dados do Secretaria no formulario
    private void vizualizarSecretaria(int indice){
        
        txtid.setText(Integer.toString(bd.getSecretarias().get(indice).getId()));
        txtnome.setText(bd.getSecretarias().get(indice).getNome());
        txtendereco.setText(bd.getSecretarias().get(indice).getEndereco());
        txtcidade.setText(bd.getSecretarias().get(indice).getCidade());
        txtbairro.setText(bd.getSecretarias().get(indice).getBairro());
        txtrg.setText(bd.getSecretarias().get(indice).getRg());
        txtcpf.setText(bd.getSecretarias().get(indice).getCpf());
        txtemail.setText(bd.getSecretarias().get(indice).getEmail());
        txttelfixo.setText(Integer.toString(bd.getSecretarias().get(indice).getTelFixo()));
        txttelcel.setText(Integer.toString(bd.getSecretarias().get(indice).getTelCel()));
        txtsalario.setText(Double.toString(bd.getSecretarias().get(indice).getSalario()));
        jdadatapagamento.setDate(bd.getSecretarias().get(indice).getDataPagamento());
        txtsenha.setText(bd.getSecretarias().get(indice).getSenha());
